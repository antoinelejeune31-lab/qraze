...
export async function GET(req: NextRequest) {
  ...
  const [
    ...
    sql`SELECT DATE(created_at AT TIME ZONE 'UTC') AS day,
        ...
        FROM download_requests WHERE created_at >= NOW() - INTERVAL '14 days'
        GROUP BY day ORDER BY day`,
    sql`SELECT EXTRACT(hour FROM created_at AT TIME ZONE 'UTC')::int AS hour,
               COUNT(*)::int AS count
        FROM download_requests GROUP BY hour ORDER BY hour`,
    ...
    sql`SELECT path, COUNT(*)::int AS count FROM page_views
        GROUP BY path ORDER BY count DESC LIMIT 8`,
    sql`SELECT DATE(created_at AT TIME ZONE 'UTC') AS day, COUNT(*)::int AS count
        FROM page_views WHERE created_at >= NOW() - INTERVAL '14 days'
        GROUP BY day ORDER BY day`,
  ])

  const total    = (totalDl[0] as any)?.n ?? 0
  const paid     = (paidDl[0]  as any)?.n ?? 0
  const convRate = total > 0 ? Math.round((paid / total) * 100) : 0

  return NextResponse.json({
    downloads: {
      total,
      today:          (todayDl[0]     as any)?.n ?? 0,
      week:           (weekDl[0]      as any)?.n ?? 0,
      uniqueEmails:   (uniqueEmails[0] as any)?.n ?? 0,
      conversionRate: convRate,
      byDay:          byDay,
      byHour:         byHour,
    },
    pageViews: {
      total:  (totalPv[0] as any)?.n ?? 0,
      today:  (todayPv[0] as any)?.n ?? 0,
      week:   (weekPv[0]  as any)?.n ?? 0,
      byPage: byPage,
      byDay:  pvByDay,
    },
  })
